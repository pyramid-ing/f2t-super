  ...
  const columns = [
    {
      ...
      render: (_: any, record: AgodaBlogJob) => (
        <Checkbox
    ...
    },
    {
      ...
      render: (_: any, row: AgodaBlogJob) => {
        return row.agodaBlogJob?.category || '-'
      },
    },
    {
      title: '발행 플랫폼',
      ...
      align: 'center' as const,
      render: (_: any, row: AgodaBlogJob) => {
        const agodaJob = row.agodaBlogJob
        if (!agodaJob) return '-'

        const platformValue = getPlatformValue(row.id, agodaJob)
    ...
    },
    {
      ...
      render: (_: any, row: AgodaBlogJob) => {
        ...
        const effectivePlatform = getPlatformValue(row.id, agodaJob) as Platform
        const showAccounts = getOptionsByPlatform(effectivePlatform)
        const value = getAccountValue(row.id, agodaJob, effectivePlatform)
        ...
        return (
          <Select
            ...
            onChange={async (accountId: number) => {
              await handleAccountChange(row.id, agodaJob, effectivePlatform, accountId)
            }}
          />
        )
      },
    },
    {
      ...
      render: (text: string, row: AgodaBlogJob) => (
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (_: any, row: AgodaBlogJob) => {
        if (row.agodaBlogJob?.resultUrl) {
          return (
            <a
              onClick={e => {
                e.preventDefault()
                window.electronAPI.openExternal(row.agodaBlogJob.resultUrl)
              }}
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (_: any, row: AgodaBlogJob) => {
        if (row.agodaBlogJob?.publishedAt) {
          return dayjs(row.agodaBlogJob.publishedAt).format('YYYY-MM-DD HH:mm')
        }
        return '-'
      },
    },
    {
      ...
      dataIndex: 'resultMsg',
      width: 350,
      render: (v: string, row: AgodaBlogJob) => {
  ...
  const baseProps: BaseJobTableProps = {
    ...
    selectionExtras: (() => {
      const selectableCount = data
        ...
        .map(j => (j as any).agodaBlogJob?.resultUrl)
...
export default AgodaBlogJobTable
