...
import { Form, Input, Button, Table, Modal, message, Switch, Space, Popconfirm, Tag } from 'antd'
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons'
import { createTistoryAccount, deleteTistoryAccount, getTistoryAccounts, updateTistoryAccount } from '../../api'
import { TistoryAccount } from '../../types/tistory'

const TistorySettingsForm: React.FC = () => {
  const [accounts, setAccounts] = useState<TistoryAccount[]>([])
  const [loading, setLoading] = useState(false)
  const [modalVisible, setModalVisible] = useState(false)
  const [editingAccount, setEditingAccount] = useState<TistoryAccount | null>(null)
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const accountsData = await getTistoryAccounts()
  ...
  const handleEditAccount = (account: TistoryAccount) => {
  ...
  const handleDeleteAccount = async (accountId: number) => {
    try {
      await deleteTistoryAccount(accountId)
  ...
  const columns = [
    ...
    },
    {
      title: '작업',
      key: 'actions',
      render: (_: any, record: TistoryAccount) => (
        <Space>
...
export default TistorySettingsForm
