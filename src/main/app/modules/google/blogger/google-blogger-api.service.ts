...
export class GoogleBloggerApiService {
  ...
  public async publish(
    ...
    // URL 매핑 적용
    const mappedPost = await this._mapPublishedUrl(response.data, bloggerBlogId)
   ...
   */
  public async getUserSelfBlogsByOAuthId(
    oauthId: number,
    accessToken: string,
  ): Promise<BloggerTypes.BloggerBlogListResponse> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.bloggerApiUrl}/users/self/blogs`, {
          headers: { Authorization: `Bearer ${accessToken}` },
   ...
   */
  private async _mapPublishedUrl(
